...
Key/IV program → derive H, build J0 (96-bit IV fast path).

AAD path → GHASH accumulates AAD blocks.

Payload path → CTR keystream ⊕ plaintext = ciphertext; GHASH accumulates ciphertext (encrypt) or plaintext (decrypt), per GCM spec.

Len block → GHASH finalization uses the 128-bit length block (len_aad || len_payload).

Tag → Compute S = GHASH(...), then Tag = E_K(J0) ⊕ S.
...
├─ gf128_reduce.v
├─ h_subkey.v
└─ third_party/secworks_aes/src/rtl/...
